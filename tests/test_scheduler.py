...
class TestSchedulerBoundarySnapshots:
    ...
    def test_media_final_prefill_boundary_keeps_full_materialized_state(
        self, mock_model, mock_tokenizer, monkeypatch
    ):
        """Media durability must not re-evaluate the terminal decode graph."""
        scheduler = Scheduler(
            model=mock_model,
            tokenizer=mock_tokenizer,
            config=SchedulerConfig(paged_cache_block_size=4),
        ...
        scheduler.block_aware_cache = MagicMock()
        request = Request(
            request_id="req-media-final-boundary",
            prompt="image",
            sampling_params=SamplingParams(),
        )
        ...
        assert calls == [True]
        assert snapshot[0] == Scheduler._PREFILL_SNAPSHOT_MARKER
        assert snapshot[1][0]["state"] == ("full",)

    def test_media_nonfinal_boundary_remains_filtered(
        self, mock_model, mock_tokenizer, monkeypatch
    ):
        """Only one full media prefix may be retained per request."""
        scheduler = Scheduler(
            model=mock_model,
            tokenizer=mock_tokenizer,
            config=SchedulerConfig(paged_cache_block_size=4),
        ...
        scheduler.block_aware_cache = MagicMock()
        request = Request(
            request_id="req-media-early-boundary",
            prompt="image",
            sampling_params=SamplingParams(),
        )
        ...
        monkeypatch.setattr(
            ...
            lambda *args, **kwargs: (_ for _ in ()).throw(
                AssertionError("early boundary captured full state")
            ),
        )
...
class TestSupportsSkipLmHead:
    ...
    def test_explicit_capability_marker_overrides_adapter_signature(self):
        class Adapter:
            supports_skip_lm_head = False
        ...
        Adapter.supports_skip_lm_head = True
        scheduler = self._scheduler_with_model(Adapter())
        assert scheduler._supports_skip_lm_head() is True
