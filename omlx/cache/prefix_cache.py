    ...
    def _update_prefix_index(
        ...
        for i, block_id in enumerate(block_ids):
            ...
            if block is None:
                ...
                break
            # The prefix index is also used by a few direct store paths that
            # have allocated blocks but have not called cache_full_blocks().
            # Record the represented token span here so a later validated
            # acquire advances the restored prefix instead of returning a
            # zero-token table.
            block.token_count = len(block_tokens)
            block_hash = block.block_hash
            if block_hash is None:
