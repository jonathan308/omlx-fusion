    ...
    class TelemetryPromptCache(original_prompt_cache):
        ...
        def prefetch_nearest_cache(self, model: Any, tokens: list[int]) -> Any:
            ...
            if result[0] is None and kv_tier is not None:
                # The durable rung of the reuse ladder: the live holder, the
                # resident slots, and the boundary-snapshot store all missed.
                # A full RAM miss is identical on every rank (same inserts,
                # same count-based eviction), so the tier's restore vote fires
                # at the same requests everywhere. A RAM hit — even a short
                # one — wins over SSD, and nothing here runs inside a model
                # collective.
