...
def _prefill(
    ...
    *,
    ...
    for start in range(0, len(values), step):
        chunk = mx.array([values[start : start + step]], dtype=mx.int32)
        positions = mx.arange(
            start,
...
def compare_qwen4_verify_window(
    ...
    *,
    ...
    exact_projection_rows: bool = True,
) -> dict[str, Any]:
    ...
    _prefill(language_model, prefix, batched_cache, step=prefill_step)
    _prefill(language_model, prefix, scalar_cache, step=prefill_step)

    verify_array = mx.array([window], dtype=mx.int32)
    if exact_projection_rows:
    ...
    left_leaves = _cache_leaves(batched_cache)
    right_leaves = _cache_leaves(scalar_cache)
    cache_reports = []
    if [name for name, _ in left_leaves] != [name for name, _ in right_leaves]:
        cache_layout_equal = False
    else:
        cache_layout_equal = True
        for (name, left), (_right_name, right) in zip(left_leaves, right_leaves):
            cache_reports.append({"name": name, **_array_comparison(left, right)})
    ...
    report = {
        ...
        "exact_projection_rows": bool(exact_projection_rows),
        "rows": rows,
        ...
        "cache_layout_equal": cache_layout_equal,
        "cache_leaves": cache_reports,
        "cache_bitwise_equal": bool(
            cache_layout_equal
            and cache_reports
            and all(item["array_equal"] for item in cache_reports)
        ),
    }
    return report
