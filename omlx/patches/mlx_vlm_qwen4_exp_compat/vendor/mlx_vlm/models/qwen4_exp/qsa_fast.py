...
def qsa_decode_selected_blocks(
    ...
    *,
    ...
    """Select chronological QSA blocks for one or more consecutive rows.

    The selector reduction/top-k is the existing decode path factored at its
    exact seam.  Callers must prove a multi-row invocation bitwise-equal to
    concatenated scalar invocations before using it as an optimization.
    """
    ...
    if (
        ...
        raise ValueError("QSA selector queries must have shape [1, M, H, D]")
    if pooled_index_keys.ndim != 3 or pooled_index_keys.shape[0] != 1:
        raise ValueError("QSA pooled selector bank must have shape [1, K, D]")
    if pooled_index_keys.shape[-1] != indexer_head_dim:
        raise ValueError("QSA pooled selector bank has the wrong head dimension")
    if compress_ratio <= 0 or token_budget <= 0 or token_budget % compress_ratio:
        raise ValueError("QSA selector token budget must contain complete blocks")
    if query_start < 0:
        raise ValueError("QSA selector query start must be nonnegative")

    rows = int(index_queries.shape[1])
    max_blocks = int(pooled_index_keys.shape[1])
    block_budget = token_budget // compress_ratio
    if max_blocks <= block_budget:
        raise ValueError("QSA selector requires a sparse block crossover")
    ...
    visible_counts = query_start + mx.arange(rows, dtype=mx.int32) + 1
    complete_counts = visible_counts // compress_ratio
    valid_blocks = (
    ...
    selected = _native_topk_indices(block_scores, block_budget)
    if selected is None:
    ...
    return mx.sort(selected, axis=-1)


def qsa_decode_from_selected_blocks(
    queries: mx.array,
    keys: mx.array,
    values: mx.array,
    selected_blocks: mx.array,
    *,
    ...
    """Run unchanged scalar decode-fast once per selected QSA row."""

    if queries.ndim != 4 or queries.shape[0] != 1 or queries.shape[2] < 1:
        raise ValueError("QSA pair queries must have shape [1, H, M, D]")
    if queries.shape[-1] != head_dim:
        raise ValueError("QSA pair queries have the wrong head dimension")
    if keys.ndim != 4 or values.shape != keys.shape or keys.shape[0] != 1:
        raise ValueError("QSA pair K/V must be matching batch-one arrays")
    rows = int(queries.shape[2])
    if selected_blocks.ndim != 3 or selected_blocks.shape[:2] != (1, rows):
        raise ValueError("QSA pair selected blocks do not match query rows")
    if query_start < 0 or query_start + rows > int(keys.shape[2]):
        raise ValueError("QSA pair query range lies outside the cache")
    ...
    for row in range(rows):
        ...
        blocks = selected_blocks[:, row].astype(mx.int32)
        selected_tokens = (
            blocks[..., None] * compress_ratio
            + mx.arange(compress_ratio, dtype=mx.int32)
        ).reshape(1, -1)
        if complete < visible:
            tail = mx.arange(complete, visible, dtype=mx.int32)[None]
        ...
        query_row = mx.contiguous(queries[:, :, row : row + 1])
        outputs.append(
            _decode_qsa_sdpa(
                query_row,
                selected_keys,
